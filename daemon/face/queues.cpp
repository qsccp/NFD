...
namespace nfd
{
    namespace face
    {
        ...
        uint32_t WDRRQueue::calObjectRate(bool getCurrentRateOnly)
        {
            ...
            // if (currentRate > 0) {
            //     std::cout << "currentRate: " << currentRate << ", " << this->recvBits << "," << this->calObjectRateInterval << ", AvaliableBandwidth: " << this->availableBandwidth << std::endl;
            // }
            bool hasCleanFlow = false;
            for (auto it = this->virtualQueues.begin(); it != this->virtualQueues.end();)
            {
                // std::cout << "key: " << it->first << ", recvBits" << it->second->recvBits << std::endl;
            ...
            for (auto &item : this->virtualQueues)
            {
                ...
                // std::cout << this->name << " => currentRate: " << item.second->currentFlowRate << ", consumerRate: " << consumeRate << ", objectRate: " << item.second->objectRate <<
                //     ", availableBandwidth" << this->availableBandwidth << ", weight: " << item.second->weight << ", totalWeight: " <<
                //     this->totalWeight << ", currentSendRate: " << currentSendRate << std::endl;
            ...
            if (consumeRate < this->availableBandwidth && hightSppedFlowNum > 0)
            {
                ...
                for (auto &item : this->virtualQueues)
                {
                    ...
                    // std::cout << this->name << " => objectRate: " << item.second->objectRate << ", remainRate: " <<
                    // remainRate << ", item.weight: " << item.second->weight << ", totalWeight: " << totalHightSpeedFlowWeight << std::endl;
                }
            }
            ...
            for (auto &item : this->virtualQueues)
            {
                ...
                if (item.second->currentFlowRate * 1.0 / item.second->objectRate > item.second->gredyRatio)
                {
                    ...
                    if (item.second->gredyTimes == item.second->gredyThreshold)
                    {
                        // std::cout << item.first << ": PenaltyFactor => " << item.second->gredyTimes << ", currentFlowRate: " << item.second->currentFlowRate << ", objectRate: " << item.second->objectRate << std::endl;
            ...
            // if (this->virtualQueues.size() > 0)
            // {
            //     std::cout << "\n\n=====================: " << this->availableBandwidth << std::endl;
            // }
            for (auto &item : this->virtualQueues)
            {
                if (item.second->penaltyFactor > 0.99)
                ...
                // std::cout << item.first << ", currentFlowRate: " << item.second->currentFlowRate
                //           << ", objectRate: " << item.second->objectRate << ", size: " << item.second->currentSize
                //           << ", weight: " << item.second->weight << ", penaltyFactor: " << item.second->penaltyFactor
                //           << std::endl;
            }
            // if (this->virtualQueues.size() > 0)
            // {
        ...
        uint32_t WFQQueue::calObjectRate(uint32_t availableBandwidth)
        {
            ...
            {
                // std::cout << "high speed flow num: " << hightSppedFlowNum << ", totalHightSpeedFlowWeight: " << totalHightSpeedFlowWeight << std::endl;
                for (auto &queue : this->queues)
                {
                    if (queue->getRemainBandwidthRate(queue->originAvaliableBandwidth) < 0.2)
                    {
                        // std::cout << "!!!" << queue->getWeight() * remain * 1.0 / totalHightSpeedFlowWeight +
                        // queue->originAvaliableBandwidth << std::endl;
            ...
            {
                // std::cout << "has flow queue num: " << hasFlowQueueNum << ", totalHasFlowQueueWeight: " << totalHasFlowQueueWeight << std::endl;
                for (auto &queue : this->queues)
                {
        ...
        bool GlobalSP::appendInterest(const Interest &interest)
        {
            ...
            // std::cout << "appendInterest => serviceClass: " << serviceClass << std::endl;
        ...
        void GlobalSP::receiveInterest(const Interest &interest)
        {
            ...
            // std::cout << this << "receiveInterest => size: " << size << ", interestBytes: " << this->interestBytes << std::endl;
        }
        ...
        void GlobalSP::calObjectRate(bool getCurrentRateOnly)
        {
            ...
            // std::cout << "AvaliableBandwidth: " << this->availableBandwidth << ", remain: " << remain << ", interestBytes" << this->interestBytes << std::endl;
        ...
        void GlobalSP::assignObjectRate(const Data &data)
        {
            ...
            // std::cout << "assignObjectRate => serviceClass: " << serviceClass << std::endl;
